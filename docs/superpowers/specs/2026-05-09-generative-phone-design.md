...
## Bootstrap UX during the wait

The Phase-A wait is ~5-12s. To keep the user (and the demo video) engaged:

1. **Stream Gemini tokens.** ADK supports streaming; we listen for partial structured output and render skeleton widgets as soon as we can identify what's coming (e.g., the moment we see `"chart"` in the tree, render a chart-shaped skeleton).
2. **Progress text.** Three short status lines that advance as bootstrap proceeds: *Designing layout…* → *Composing widgets…* → *Almost ready…* (and for B3 apps: *Starting backend…* visible after the app loads).
3. **Animated phone-installing visual.** While bootstrap runs, the new app's icon appears on the home screen with a circular progress ring around it — like iOS app install. When Phase A returns, ring completes, icon settles, screen transitions into the app.

The wait is not eliminated, but it is *visually filled* and reads as the agent doing real work, not the app being broken. Phase B (sandbox) is invisible to the user — they're already operating the app on seed data while Daytona warms up.
...
3. ~8s with streaming skeleton + animated install ring on the home-screen icon. Screen transitions to the new app: title "Marathon", running-figure icon, weekly mileage chart, calendar of planned runs, race countdown ring, recent runs list with seed data.
3a. (B3 only) ~10-15s after the app appears, the chart silently refreshes with sandbox-computed data — optional visible "live" badge to highlight that a generated backend just came online.
...
Built on `@google/adk` (TypeScript ADK). ADK manages the function-calling loop internally — we declare an `LlmAgent` with `FunctionTool`s and a system instruction; ADK handles the iterate-call-Gemini-execute-tool-loop-until-final-text cycle. We never write that loop ourselves.

Two distinct invocations:

#### Bootstrap (Phase A — blocking, ~5-12s)

- **Input:** user's first message in a new chat thread.
- **Output (structured):** `{ name, icon, tree, data, tools, backendCode? }` where `tools` is the per-app function catalog declarations and `backendCode` is the optional B3 Python source.
- **Implementation:** a one-shot `LlmAgent` invocation with no tools (it's pure structured output), system instruction includes the widget catalog and the JSON schema.
- **Returns to client immediately** with seed data. Client renders app while Phase B (if applicable) runs in the background.

#### Sandbox provisioning (Phase B — async, ~15-20s, B3 only)

- Triggered server-side immediately after Phase A returns to the client. Does NOT block the bootstrap response.
- Server: `daytona.create()` → `uploadFile(app.py)` → `pip install` → start service → poll health → store `backendUrl` on the app record.
- Client polls `GET /api/app/:id/backend` every 2s until status flips from `provisioning` → `ready`. Once ready, widgets bound to `$BACKEND/...` start fetching live data; the seed-data render swaps to live without user interaction.
- If provisioning fails (Daytona timeout, broken Python, network), backend status flips to `failed` and widgets stay on seed data permanently. The app still works.

#### Mutate (per chat turn, demo-required)

- **Input:** user's chat message in an existing app.
- **Implementation:** an `LlmAgent` invocation with this app's tools (declared at bootstrap, instantiated as `FunctionTool` objects on the server). ADK runs the loop: call Gemini → execute any tool calls against a working copy of `data` → loop until Gemini returns final text → return updated `data` + history + assistant message to the client.
- **Per-app tool catalog:** `addRun`, `removeRun`, `setGoalRace`, `planWeek` for marathon; `addDay`, `addActivity`, `setBudget`, `movePin` for trip; `addApplication`, `moveStage`, `addContact`, `logEvent` for job tracker. Server has a registry of FunctionTool implementations; bootstrap output names which apply to this app.
- **Server is stateless.** History lives on the app record in the browser's localStorage and is sent in/out with each mutate call.

The simplification ADK buys us: we don't parse Gemini response parts, don't construct `functionResponse` messages, don't manage the iteration cap. ADK does. The user has tried hand-rolling this loop before; this is non-negotiable.
...
| Agent framework | `@google/adk` (Google ADK for TypeScript, v1.0+) — manages the function-calling loop, sessions, and streaming. `@google/adk-devtools` for the local debug UI (`npx adk web`) during development |
| LLM | Gemini 3 Pro via ADK's Vertex AI binding (verify exact model ID at hour 0). Fall back to Gemini 3 Flash if Pro latency exceeds the 8s Phase-A budget |
| Observability | `langsmith` SDK — `traceable` wrapper at the ADK invocation boundary so every agent run is one trace with all tool calls nested |
...
| Generation latency too slow for demo | medium | Phase A targeted at ≤8s — if Gemini Pro misses, fall back to Flash. Stream tokens, render skeleton on first identifiable widget. Phase B (sandbox) is async and never blocks the user. |
| Backend-status polling chatty / racy | low | Poll every 2s with exponential backoff to 5s after 30s; cap at 90s before flipping status to `failed`. Single endpoint, idempotent. |
...
- Exact Gemini model ID — verify against current Vertex AI / Google AI Studio offerings when ADK is wired up at hour 0. Default to current Gemini 3 Pro; switch to Flash if Pro latency exceeds the ~8s Phase-A budget.
- ADK structured-output API — ADK 1.0 GA exposes structured output but exact pattern (Zod schema vs raw JSON schema vs `responseSchema` passthrough) needs to be confirmed against `@google/adk` v1.1+ docs at hour 0.
